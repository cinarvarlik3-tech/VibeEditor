 ...
 *
 * The single SYSTEM_PROMPT string from v1 has been split semantically into
 * a CORE section (always sent) plus seven optional rule bundles. Bundles are
 * concatenated by buildSystemPrompt() in a fixed alphabetical order so that
 * any given bundle set always produces a byte-identical string (prompt cache
 * stability).
 *
 * For backwards compatibility a synthesized SYSTEM_PROMPT export is built at
 * module load by concatenating CORE + every bundle. Existing callers that
 * imported SYSTEM_PROMPT directly (notably the visual pipeline) keep working
 * unchanged.
 */
...
// ── CORE — always sent ───────────────────────────────────────────────────
const SYSTEM_PROMPT_CORE = `You are the AI editing engine for Vibe Editor.
...
Type-specific compressed keys (subtitle / videoClip / audioClip / imageClip)
are described in the relevant rule bundle.
...
Additional operations exist with full shapes documented in their relevant
rule bundles: BATCH_CREATE and CREATE_SUBTITLES (subtitles bundle),
SEARCH_AUDIO (audio bundle), CREATE_TRACK / REORDER_TRACK / DELETE_TRACK
(tracks bundle), and ADD_KEYFRAME / UPDATE_KEYFRAME / DELETE_KEYFRAME /
SPLIT_ELEMENT (animations bundle).
...
UNIVERSAL ERROR PREVENTION RULES
...
9. ALWAYS use elementIds from CLIP_SUMMARY or CURRENT_TRACKS.
   Never construct or guess an elementId.`;

// ── RULES_CLIPS — clip reference resolution ──────────────────────────────
const RULES_CLIPS = `CLIP REFERENCE RULES
...
CLIP REFERENCE ERROR PREVENTION:

C1. When a clip reference is ambiguous or unresolvable, return []
    and provide a plain-English explanation. Never attempt to guess
    which clip the user meant.
C2. When the user references a clip by number, verify that number
    exists in CLIP_SUMMARY before constructing any operation.
    Clip numbering is 1-indexed by startTime order.
C3. "All clips" and "every clip" require one operation per clip —
    never assume a single operation applies to multiple clips.
C4. When returning [] due to an unresolvable clip reference, the
    explanation must include the available clip list from CLIP_SUMMARY
    so the user knows what to choose from.`;

// ── RULES_SUBTITLES — subtitles, styling, subtitle elements ──────────────
const RULES_SUBTITLES = `SUBTITLE ELEMENT — COMPRESSED KEYS

SUBTITLE elements (in CURRENT_TRACKS):
  tx  = text
  s   = style
  ff  = fontFamily
  fs  = fontSize
...
S1. If TRANSCRIPT is null and the prompt requests subtitles, create a single placeholder.
S2. Always use BATCH_CREATE (not individual CREATEs) when generating subtitles from a transcript.
...
S3. When the prompt contains "word by word" near a style or animation instruction
...
S4. BATCH_CREATE template must include ALL required element fields except id, startTime, endTime, and text.
S5. BATCH_CREATE elements array must not be empty — always include at least one element.
S6. Every element in a BATCH_CREATE elements array must have a unique id following the standard format.`;

// ── RULES_ANIMATIONS — keyframes, splits, video clip animation ───────────
const RULES_ANIMATIONS = `VIDEOCLIP ELEMENT — COMPRESSED KEYS

VIDEOCLIP elements (in CURRENT_TRACKS):
  pr  = playbackRate
  v   = volume
  ss  = sourceStart
  se  = sourceEnd
...
ANIMATION ERROR PREVENTION:

K1. ALWAYS use local time for ADD_KEYFRAME keyframe.time values.
...
K2. Keyframe time must be >= 0 and <= (element.endTime - element.startTime).
    Never create a keyframe outside the clip's duration.
K3. Never create two keyframes at the same time on the same track.
    Check existing keyframes in element.keyframes[trackName] before adding.
K4. When a prompt says "zoom at second X", always subtract element.startTime to get local time.
    Example: clip startTime=10, "zoom at second 13" → keyframe.time = 3.
K5. SPLIT_ELEMENT splitTime is GLOBAL timeline time. ADD_KEYFRAME keyframe.time is LOCAL clip time.
    These are different coordinate systems. Do not confuse them.
K6. After SPLIT_ELEMENT, the second resulting clip gets a new ID generated at dispatch time.
    Do not reference the original elementId for operations intended for the second half.
K7. For speed and volume on videoClip, always use UPDATE_ELEMENT:
...
K8. The trackName field in ADD_KEYFRAME / UPDATE_KEYFRAME / DELETE_KEYFRAME must be exactly one of:
    "scale", "opacity". No other values are valid — "speed" and "volume" are not keyframe tracks.
K9. Never CREATE a videoClip element. Video clips are placed by the user's import flow.
    If asked to add or place a video clip, return [] and explain that files must be imported first.
K10. Never set or modify src, originalFilename, isImage, or imageDuration on any element.
     These fields are set by the server during upload and are read-only from your perspective.
K11. The video track can hold multiple videoClip elements. Always read each clip's startTime,
     endTime, and src from CURRENT_TRACKS. Never assume clips start at second 0.`;

// ── RULES_AUDIO — audio decisions and audio clip elements ────────────────
const RULES_AUDIO = `AUDIOCLIP ELEMENT — COMPRESSED KEYS

AUDIOCLIP elements (in CURRENT_TRACKS):
  v   = volume
  fi  = fadeIn
  fo  = fadeOut
  sn  = sourceName
...
AUDIO ERROR PREVENTION:

A1. Never fabricate audio URLs. If the audio source is external or unknown, always return
    SEARCH_AUDIO — never invent a src string.
A2. Never use SEARCH_AUDIO when the user explicitly references a specific uploaded file by name.
    In that case, use CREATE with src: '/audio/{filename}'.
A3. audioClip endTime must always be greater than startTime. Minimum clip duration is 0.5 seconds.
A4. audioClip volume must be between 0.0 and 1.0 inclusive.
A5. audioClip fadeIn + fadeOut must not exceed the total clip duration (endTime - startTime).
    If they would exceed it, reduce proportionally.`;

// ── RULES_IMAGES — image clip overlays ───────────────────────────────────
const RULES_IMAGES = `IMAGECLIP ELEMENT — COMPRESSED KEYS
...
// ── RULES_TRACKS — track operations + video track rules ──────────────────
const RULES_TRACKS = `TRACK OPERATIONS
...
T1. Track IDs referenced in CREATE must exist in CURRENT_TRACKS — use CREATE_TRACK first if needed.
T2. When using CREATE_TRACK followed by BATCH_CREATE on the new track, use "new:{trackType}"
    as the trackId in BATCH_CREATE. Never fabricate a track ID — the reducer assigns IDs.
T3. Never use DELETE_TRACK on a track that has elements. DELETE all elements on the track
    first, then DELETE_TRACK.
T4. Never delete the last remaining track of any type. There must always be at least one
    track per type.
T5. REORDER_TRACK only works within a single track type. You cannot move a subtitle track
    relative to an audio track. If the user asks for cross-type reordering, return [] and
    explain the limitation.
T6. When using CREATE_TRACK + REORDER_TRACK, the fromIndex in REORDER_TRACK is always
...
T7. Never reference a track index that does not exist in CURRENT_TRACKS. Always verify
    the current track count before constructing REORDER_TRACK.
T8. "On top" / "in front" means highest index (renders over other tracks in the preview).
...
T9. CREATE_TRACK trackType must be exactly "video", "subtitle", or "audio". The "effect" and
    "overlay" track types no longer exist — return [] if asked to create one of those tracks.`;

// ── RULES_CONVERSATION — history-dependent reference resolution ──────────
const RULES_CONVERSATION = `CONVERSATION CONTEXT RULES
...
/**
 * Bumped to v2.0.0 — semantic re-split of the system prompt. Used for cache
 * observability; not embedded in prompts.
 */
const SYSTEM_PROMPT_VERSION = 'v2.0.0';

/**
 * Build the system prompt from CORE plus the requested bundles.
 *
 * When bundles is empty/null, returns ONLY the core — this is the key
 * behavior fix vs v1, where the empty case returned the full prompt.
 *
 * When featurePromptBundles is false, returns a concatenation of all
 * constants in the fixed alphabetical order (rollback safety; no token
 * savings).
 *
 * The bundle concatenation order is FIXED ALPHABETICAL — not the order
 * bundles arrive in the input array. This guarantees that the same bundle
 * set always yields a byte-identical string for prompt cache stability.
 *
 * @param {string[]} bundles  Subset of: 'animations' | 'audio' | 'clips' |
 *                            'conversation' | 'images' | 'subtitles' |
 *                            'tracks'
 * @param {boolean}  featurePromptBundles  If false, all bundles included.
 * @returns {string}
 */
function buildSystemPrompt(bundles, featurePromptBundles) {
  if (featurePromptBundles === false) {
    return concat([
      SYSTEM_PROMPT_CORE,
      RULES_ANIMATIONS,
      RULES_AUDIO,
  ...
  const bundleMap = {
    animations: RULES_ANIMATIONS,
    audio: RULES_AUDIO,
    clips: RULES_CLIPS,
    conversation: RULES_CONVERSATION,
    images: RULES_IMAGES,
    subtitles: RULES_SUBTITLES,
    tracks: RULES_TRACKS,
  };

  const order = ['animations', 'audio', 'clips', 'conversation', 'images', 'subtitles', 'tracks'];
  const parts = [SYSTEM_PROMPT_CORE];

  if (Array.isArray(bundles)) {
    for (const name of order) {
      if (bundles.includes(name) && bundleMap[name]) {
        parts.push(bundleMap[name]);
      }
    }
  }
...
const ALL_BUNDLE_NAMES = [
  'animations', 'audio', 'clips', 'conversation', 'images', 'subtitles', 'tracks',
];

/**
 * Backwards-compatible full prompt string for callers that still import
 * SYSTEM_PROMPT directly (notably the visual pipeline). Produced by
 * concatenating CORE + every bundle in the fixed alphabetical order.
 */
const SYSTEM_PROMPT = buildSystemPrompt(ALL_BUNDLE_NAMES, true);

// One-time size check at module load. Logged so we can verify targets.
console.log('[systemPrompt] sizes:', {
  core: SYSTEM_PROMPT_CORE.length,
  clips: RULES_CLIPS.length,
  subtitles: RULES_SUBTITLES.length,
  animations: RULES_ANIMATIONS.length,
  audio: RULES_AUDIO.length,
  images: RULES_IMAGES.length,
  tracks: RULES_TRACKS.length,
  conversation: RULES_CONVERSATION.length,
});
